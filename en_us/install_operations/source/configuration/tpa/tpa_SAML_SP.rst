...
configure your Open edX installation as a SAML SP, which creates your metadata
XML file.
...
#. Add the ``EDXAPP_SOCIAL_AUTH_SAML_SP_PUBLIC_CERT`` parameter.
...
#. Open the ``saml.crt`` file, copy the entire contents, and then paste them
   onto the line after the public cert parameter.
...
#. Add the ``EDXAPP_SOCIAL_AUTH_SAML_SP_PRIVATE_KEY`` parameter.
...
#. Open the ``saml.key`` file, copy the entire contents, and then paste them
   onto the line after the private key parameter.
...
#. Select **Save** at the bottom of the page. You can direct identity providers
